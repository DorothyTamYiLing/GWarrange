...
Location of repeat sequence candidates (obtained above) in the genomes are obtained through BLAST. Multiple sequences can be placed in the same multifasta file for obtaining their genome locations in all genomes at once.
```
...
```
makeblastdb -in example_data/clus1clus2_47.fna -dbtype nucl -out genome_db
```
...
```
Note that the value used for -flk_dist parameter should be larger than the "Maximum size of merged ISs" value in the corresponding *mergedISstat.txt file that was generated by merge_IS.R.
