...
#set the height of the ceiling depends on the total number of case and control genomes for plot
mymagn<-1000

ctrl_num<-sum(mymed_ctrl_R$count) #get the number of ctrl genome for plot
case_num<-sum(mymed_case_R$count) # get the number of case genome for plot

if(ctrl_num>case_num | case_num==ctrl_num){
  myheight<-ctrl_num*mymagn #this number should be the same as that in calculating sizefactor
}
if(case_num>ctrl_num){
  myheight<-case_num*mymagn #this number should be the same as that in calculating sizefactor
}

case_count<-10000 #set the starting y axis level
ctrl_count<--10000 #set the starting y axis level
...
for(k in 1:nrow(mymed_ctrl_L)){
  ...
  sizefactor<-mymed_ctrl_L$count[k]*mymagn
...
ctrl_count<-ctrl_count+(-myheight/2)-(max(mymed_ctrl_L$count)*500/2)-50 #set the starting y axis level
...
for(k in 1:nrow(mymed_ctrl_R)){
  ...
  sizefactor<-mymed_ctrl_R$count[k]*mymagn
...
for(k in 1:nrow(mymed_case_L)){
  ...
  sizefactor<-mymed_case_L$count[k]*mymagn
...
case_count<-case_count+(myheight/2)+(max(mymed_case_L$count)*500/2)+50 #set the starting y axis level
...
for(k in 1:nrow(mymed_case_R)){
  ...
  sizefactor<-mymed_case_R$count[k]*mymagn
...
plot(1, type="n", xlim=c(1,5000), ylim=c((ctrl_count+(-myheight/2)-(sizefactor/2)),(case_count+(myheight/2)+(sizefactor/2))), xlab="genome position (thousands)",ylab="genome_index")

abline(h=0) #to separate the case from control kmers
text(5000,3000,"case",cex=0.7)
text(5000,-3000,"control",cex=0.7)
